...
class Dynamic_menu {
    ...
    function build_menu($type) {
        ...
        $query = $this->ci->db->query("select * from prak_menu");

        // now we will build the prakamic menus.
        ...
        foreach ($query->result() as $row) {
            ...
            $url = $row->url;
            $uri = $row->uri;
            $prak_group_id = $row->prak_group_id;
    ...
    function get_childs($id) {
        ...
        $query = $this->ci->db->query("select * from prak_menu where parent_id = $id");
